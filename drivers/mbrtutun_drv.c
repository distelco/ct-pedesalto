/*
	...
	$Id: mbrtutun_drv.c,v 1.2 2009-01-29 10:32:55 matteoc Exp $
*/
...
*/
static int modbusRTUtun_read_digital( void * data, char *buf, int buflen )
{
	...
	{
		DEBUG_LOG("modbusRTUtun_read_di:connect in progress %s",pdata->tcp->ip);
		return -1;
	}
	...
	{
		ERR_LOG("modbusRTUtun_read_di:Richiesta invalida" ) ;
	}
	
	//spedisco ed aspetto la risposta 
	if ( mbRTUtun_send( pdata->tcp->sock, pdata->slaveid, pdu, resp, pdata->tcp->timeout) < 0)
	{
		//gestisco un eventuale errore
		ERR_LOG("modbusRTUtun_read_di:Impossibile inviare il pacchetto");
...
static int modbusRTUtun_write_digital( void * data, char *buf, int buflen )
{
	...
	if ((tcp_connect(pdata->tcp)) < 0)
	{
		DEBUG_LOG("modbusRTUtun_write_do:connect in progress %s",pdata->tcp->ip);
		return -1;
	}
	det.start_addr = pdata->starting_addr;	//remote data starting address
	det.num = how_many;		//request data number
	...
	if ( mbRTUtun_send( pdata->tcp->sock, pdata->slaveid, pdu, resp, pdata->tcp->timeout) < 0)
	{
		//gestisco un eventuale errore
		//free(det.value);
		ERR_LOG("modbusRTUtun_write_do:Impossibile inviare il pacchetto");
		tcp_disconnect(pdata->tcp);
		return -1;
	}
	return 0;
...
/*
	...
	Return value: 0 Success, -1 error
*/
static int modbusRTUtun_read_analog( void * data, long *buf, int buflen )
{
	BYTE pdu[5];		// Application Data Unit
	BYTE resp[BUF_SIZE];	//contiene la risposta del server, cioe' i dati letti oppure errore
	struct S_req_details det;
	...
	if ((tcp_connect(pdata->tcp)) < 0)
	{
		DEBUG_LOG("modbusRTUtun_read_ai:connect in progress %s", pdata->tcp->ip);
		return -1;
	}
	...
	{
		//gestisco un eventuale errore
		ERR_LOG("modbusRTUtun_read_ai:Impossibile inviare il pacchetto");
		tcp_disconnect(pdata->tcp);
		return -1;
	}
...
/*
	...
	Return value: 0 Success, -1 error
*/
static int modbusRTUtun_write_analog( void * data, long *buf, int buflen )
{
	BYTE pdu[32];		// Application Data Unit
	BYTE resp[BUF_SIZE];	//contiene la risposta del server, cioe' i dati letti oppure errore
	BYTE send[BUF_SIZE];   
	...
	if ((tcp_connect(pdata->tcp)) < 0)
	{
		DEBUG_LOG("modbusRTUtun_write_ao:connect in progress %s",pdata->tcp->ip);
		return -1;
	}
	det.start_addr = pdata->starting_addr;	//remote data starting address
	det.num = how_many;		//request data number
	...
	if ( mbRTUtun_send( pdata->tcp->sock, pdata->slaveid, pdu, resp, pdata->tcp->timeout) < 0)
	{
		//gestisco un eventuale errore
		//free(det.value);
		ERR_LOG("modbusRTUtun_write_ao:Impossibile inviare il pacchetto");
		tcp_disconnect(pdata->tcp);
		return -1;
	}
	return 0;
...
static int modbusRTUtun_init_block()
{
	return 0;
}

struct S_driver_descriptor mbrtutun_io_driver_descriptor =
{
	"io remoti",		//	char name[64];
	drv_mbrtutun_io_did,	//	long did ;
	modbusRTUtun_read_digital,	//	int (* read_di)   ( void * data, char *buf, int buflen )
	modbusRTUtun_write_digital,	//	int (* write_do)  ( void * data, char *buf, int buflen );
	modbusRTUtun_read_analog,	//	int (* read_ai)   ( void * data, long *buf, int buflen );
	modbusRTUtun_write_analog,	//	int (* write_ao)  ( void * data, long *buf, int buflen );
	NULL,			//	int (* read_mem)  ( void * data, char *buf, int buflen );
	NULL,			//	int (* write_mem) ( void * data, char *buf, int buflen );
	modbusRTUtun_init_block,	//	int (* init_block)( struct S_driver_io_block * 	 , FILE* );
