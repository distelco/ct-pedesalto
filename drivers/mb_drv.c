/*
	...
	$Id: mb_drv.c,v 1.1.1.1 2005-12-28 17:21:22 matteoc Exp $
*/
...
*/
int modbus_read_digital( void * data, char *buf, int buflen )
{
	...
	if ( mbsend_eth( pdata->tcp->sock, pdu, resp, pdata->tcp->timeout) < 0)
	{
...
int modbus_write_digital( void * data, char *buf, int buflen )
{
...
/*
	...
	Return value: 0 Success, -1 error
*/
int modbus_read_analog( void * data, long *buf, int buflen )
{
	BYTE pdu[5];		// Application Data Unit
	BYTE resp[BUF_SIZE];	//contiene la risposta del server, cioe' i dati letti oppure errore
	struct S_req_details det;
...
/*
	...
	Return value: 0 Success, -1 error
*/
int modbus_write_analog( void * data, long *buf, int buflen )
{
	BYTE pdu[32];		// Application Data Unit
	BYTE resp[BUF_SIZE];	//contiene la risposta del server, cioe' i dati letti oppure errore
	BYTE send[BUF_SIZE];   
...
int modbus_init_block()
{
	return 0;
}

struct S_driver_descriptor ul_driver_descriptor =
{
	"unita' locale",	//	char name[64];
	drv_ul_did,		//	long did ;
...
struct S_driver_descriptor cl_driver_descriptor =
{
	"centrale locale",	//	char name[64];
	drv_cl_did,		//	long did ;
	modbus_read_digital,	//	int (* read_di)   ( void * data, char *buf, int buflen )
	modbus_write_digital,	//	int (* write_do)  ( void * data, char *buf, int buflen );
	modbus_read_analog,	//	int (* read_ai)   ( void * data, long *buf, int buflen );
